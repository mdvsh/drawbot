...
void Init(Robot *rob)
{
    ...
    kalman_init(&rob->heading_filter);
};
...
void UpdateDirection(Robot *rob)
{
    ...
    if (!new_point.valid) {
        rob->active = false;
        rob->pen_up = true;
        buzzer_set_state(BUZZER_COMPLETE);
        return;
    }

    buzzer_set_state(BUZZER_DRAWING);
    ...
    //    float gyro_rate = gyro_get_dps(2);
    //    imu_Data imu_data;
    //    imu_read_all(&imu_data);
    //    float mag_heading = imu_get_heading(&imu_data);
    //    float filtered_heading = kalman_update(&rob->heading_filter, gyro_rate, mag_heading);
    //	float heading_error = rob->desired_heading - filtered_heading;
    ...
    if (rob->desired_heading < 0)
        rob->desired_heading += 360;
    rob->active = true;
...
void ReadUart(Robot *rob, uint16_t x, uint16_t y, uint16_t pen)
{
    ...
    point new_point = {.x = x, .y = y, .pen_up = pen_up, .valid = true};
