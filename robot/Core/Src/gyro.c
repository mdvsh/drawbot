...
static uint8_t read_register(uint8_t reg_addr) {
    uint8_t value;
    HAL_I2C_Mem_Read(&hi2c1, L3GD20H_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, HAL_MAX_DELAY);
    return value;
}

static void write_register(uint8_t reg_addr, uint8_t value) {
    HAL_I2C_Mem_Write(&hi2c1, L3GD20H_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, HAL_MAX_DELAY);
}
...
void gyro_calibrate(void) {
    const int NUM_SAMPLES = 500;  // More samples
    int32_t sum[3] = {0, 0, 0};
    ...
    if(dt > 0.0f && dt < 0.5f) {
        ...
        if(fabs(dps) > 0.1f) {
            ...
            if((now - last_print) > 500) {
                printf("DPS: %.2f (Raw: %.2f), Heading: %.2f\r\n", filtered_dps, dps, current_heading);
                last_print = now;
            }
        }
