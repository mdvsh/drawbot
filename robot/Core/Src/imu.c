...
static uint8_t read_register(uint8_t reg_addr) {
    uint8_t value;
    HAL_I2C_Mem_Read(&hi2c1, LSM303D_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, HAL_MAX_DELAY);
    return value;
}

static void write_register(uint8_t reg_addr, uint8_t value) {
    HAL_I2C_Mem_Write(&hi2c1, LSM303D_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, HAL_MAX_DELAY);
}
